#!/usr/bin/env python3
"""
Batch translate subtitle text via DeepSeek API. Accepts .txt or .srt input.
...
def main():
    parser = argparse.ArgumentParser(
        description="Split subtitle text into chunks, translate via DeepSeek API, aggregate results"
    )
    parser.add_argument("input_file", help="Path to input .srt or English .txt file")
    parser.add_argument("--job-id", default="", help="Job ID for DB progress updates")
    parser.add_argument("--db-path", default="", help="Path to jobs.db for progress updates")
    ...
    def _inc_progress():
        if not args.job_id or not args.db_path:
            return
        with db_lock:
            try:
                ...
                row = conn.execute("SELECT progress FROM jobs WHERE id=?", (args.job_id,)).fetchone()
                if row and "/" in (row[0] or ""):
                    cur, total_str = row[0].split("/", 1)
                    new_val = f"{int(cur) + 1}/{total_str}"
                    conn.execute("UPDATE jobs SET progress=? WHERE id=?", (new_val, args.job_id))
                conn.commit()
                conn.close()
            except Exception:
                pass
    ...
    if args.job_id and args.db_path:
        try:
            conn = sqlite3.connect(args.db_path)
            conn.execute("UPDATE jobs SET progress=? WHERE id=?", (f"0/{chunk_count}", args.job_id))
            conn.commit()
            conn.close()
        except Exception:
            pass
